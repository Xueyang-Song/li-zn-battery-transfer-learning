...
While Selective MT-GP does not achieve lower RMSE than a direct
Zn-ion GP (GP-Direct), it provides better-calibrated uncertainty
...
improvement). These results suggest that cross-chemistry transfer learning's
primary value may lie in uncertainty calibration rather than point accuracy, with
...
An unexpected result is that Shuffled MT-GP
...
result has a reasonably precise explanation grounded in the structure of GP
inference.
...
Shuffled MT-GP destroys the Li-ion regression signal, so it cannot pull ZIB predictions toward the Li-ion posterior mean. This avoids a harmful bias when the inter-task mean offset is large, explaining why Selective MT-GP's residual mean-regression toward the LIB prior slightly degrades RMSE at small $N$.

This suggests that in cross-chemistry battery transfer learning,
the feature distribution of the source domain may be more valuable than its labels
at small $N$. Future work might explore unsupervised domain
adaptation techniques \cite{pan2010transfer} (contrastive pre-training, optimal transport domain
...
in the risk of undetected overconfidence. Reporting NLL and
...
deep kernel learning or neural process models. \textbf{Second}, this study uses only coin-cell format ZN cells; transferability conclusions may not generalize to pouch or cylindrical ZIBs. \textbf{Third}, the feature selection is motivated by physical reasoning but was also guided by post-hoc analysis, introducing potential hindsight bias; prospective validation on a withheld dataset would strengthen the causal interpretation. \textbf{Fourth},
