...
cycling protocols \cite{severson2019, paulson2022, zhu2020tincone}, ZIB lifecycle datasets
...
Figure~\ref{fig:overview} summarizes the full pipeline from raw cycling data
to calibrated predictions.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{../figures/fig1_method_overview.pdf}
  \caption{Overview of the Selective MT-GP pipeline.
  Li-ion source cells (blue) and Zn-ion target cells (orange) each undergo
  chemistry-specific feature extraction. The exponential decay rate
  $\text{exp\_b}$ is identified as a transferable feature (shared kernel,
  green), while the differential capacity variance $\Delta\mathrm{var}(dQ/dV)$
  is treated as a Zn-ion-private feature (red) because its absolute scale and
  mechanistic origin differ fundamentally across chemistries. The two kernel
  components are combined additively ($k_{\mathrm{sel}} = k_{\mathrm{shared}}
  + k_{\mathrm{private}}$) to form the Selective MT-GP, which outputs a
  full predictive posterior $\mu^*(N_\mathrm{cyc}) \pm \sigma^*(N_\mathrm{cyc})$
  in log-cycle space. Zn-ion cells additionally require PELT-based activation
  detection before feature extraction.}
  \label{fig:overview}
\end{figure}
...
the support of the source domain and are relevant to understanding the
...
the nominal 90\%. The underlying mechanism is that at $N = 2$, the
...
our results appear to support empirically.
...
and Zn anode plating/stripping --- a different set of
...
A somewhat counterintuitive finding in this work is that Shuffled MT-GP
...
More broadly, these results point toward a recurring challenge in applying
machine learning across physical domains: \textbf{physical feature
alignment must precede data-driven label transfer}. Identifying which features
...
neural networks --- represent a promising direction for future work. Additional
directions include neural process architectures for non-linear cross-chemistry
meta-learning, temperature-stratified transfer for thermal generalization, and
